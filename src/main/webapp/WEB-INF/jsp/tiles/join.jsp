				...
				<fieldset>
					...
					<ul class="app_list">
						...
						<li class="clear">
							...
							<div class="app_content"><input type="text" name="PASSWORD_HINT" class="w100p" id="password_hint_lbl" placeholder="암호 힌트를 입력해주세요" required/></div>
						</li>
						<li class="clear">
							<label for="password_cnsr_lbl" class="tit_lbl pilsoo_item">힌트 답변</label>
							<div class="app_content"><input type="text" name="PASSWORD_CNSR" class="w100p" id="password_cnsr_lbl" placeholder="힌트 답변을 입력해주세요" required/></div>
						</li>
						<li class="clear">
							<label for="user_nm_lbl" class="tit_lbl pilsoo_item">이름</label>
							<div class="app_content"><input type="text" name="USER_NM" class="w100p" id="name_nm_lbl" placeholder="이름을 입력해주세요" required/></div>
						</li>
						<li class="clear">
						...
						<li class="clear">
							...
							<div class="app_content"><input type="text" name="HOUSE_ADRES" class="w100p" id="house_adres_lbl" placeholder="주소를 입력해주세요" required/></div>
						</li>
						<li class="clear">
					...
					<p class="btn_line">
					* 관리자의 가입 승인 후, 이용 가능합니다.
					<p class="btn_line">
					<button disabled style="opacity:0.7" id="btn_join" type="submit" class="btn_baseColor">회원가입</button>
					</p>	
				</fieldset>
...
$(document).ready(function(){
	...
	$("#EMPLYR_ID").bind("blur", function() {
		...
		if(emplyr_id == ''){
			...
			$("#btn_join").attr("disabled", true); //submit버튼 비활성화
			$("#btn_join").css("opacity", "0.5");
			$("#btn_join").css("cursor", "default");
			return false;
		}
		$.ajax({
			...
			success:function(result){
				if(result == "0") { //중복ID가 없다면
					alert("사용 가능한 아이디입니다.");
					$("#btn_join").attr("disabled", false); //submit버튼 활성화
					$("#btn_join").css("opacity", "1.0");
